...
//
//functions for users
func InsertUsers(id int, password string, name string) error {
	bkey := []byte(password)
...
func CheckUsers(id int, password string) (bool, string) {
	bkey := []byte(password)
	...
	if exist == false {
		...
		qsu.Filter("id", id).All(&find, "hashed_key", "name")
		findhashed = []byte(find.Hashed_key)
		returnname = find.Name
	}
	err := bcrypt.CompareHashAndPassword(findhashed, bkey)
	if err == nil {
		return true, returnname
	} else {
		return false, ""
	}
}
func ConnectDB(username string, password string, dbname string, site string) error {
	err := orm.RegisterDriver("mysql", orm.DRMySQL)
	if err != nil {
		fmt.Println("Driver register error :", err)
	}
	err = orm.RegisterDataBase("default", "mysql", username+":"+password+"@tcp("+site+")/"+dbname+"?charset=utf8")
	if err != nil {
		fmt.Println("Register DataBase error : ", err)
	}
