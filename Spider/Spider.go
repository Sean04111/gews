...
//The Sipder was used to designed for sina news @version1.0
//Now It is for Tik Tok @version1.5
...
func VisitMain(url string) ([60]string, [60]string) {
	var alltext [60]string
	var allhref [60]string
	client := &http.Client{}
	reqest, _ := http.NewRequest("GET", url, nil)
	response, _ := client.Do(reqest)
	docu, _ := goquery.NewDocumentFromReader(response.Body)
	docu.Find("body > div.main.wide > div.row1 > div.col-17.col-m-24.col1- > div:nth-child(1) > div.tx-text.f-16.mb15 > div > div > li ").Each(func(i int, s *goquery.Selection) {
		a := s.Find("span:nth-child(2) > a")
		href, _ := a.Attr("href")
		text := a.Text()
		alltext[i] = text
		allhref[i] = "http://resou.today" + href
	})
	return alltext, allhref
}

//return the text and the site of the imgs
func VisitSingleNew(url string) ([100]string, [100]string) {
	var text [100]string
	var img [100]string
	client := &http.Client{}
	reqest, _ := http.NewRequest("GET", url, nil)
...
func Parse(html string) {
	...
	regexp_sidebar := regexp.MustCompile(`<link rel="preload" href="(.*?)" as="image" id="preloadBg">`)
...
func GetBackImgFromBing() string {
	bing := "https://www.bing.com/"
	client := &http.Client{}
	reqest, _ := http.NewRequest("GET", bing, nil)
	response, _ := client.Do(reqest)
	docu, _ := goquery.NewDocumentFromReader(response.Body)
	s := docu.Find("#preloadBg")
	href, _ := s.Attr("href")
	return bing + href
}
